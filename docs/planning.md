...
│   │
│   ├── ScheduleView.tsx      # 일정 주간 리스트 뷰 (날짜별 그룹)
│   ├── AddEventButton.tsx
│   ├── EditEventDialog.tsx
│   │
...
메인 페이지
  ...
  │
  └─ 🗓️ 일정 탭
      ├─ 날짜별 그룹 리스트 (오늘/내일/어제 강조)
      ├─ 카드 왼쪽 색띠로 주인 구분 (🩷 수진/분홍 / 💙 종빈/하늘 / 💛 SJB/노랑)
      ├─ 지난 일정 접기/펼치기
      └─ ➕ FAB: 일정 추가 (제목 + 날짜 + 시간/종일 + 주인 3택 + 메모)
```
...
- **뷰 형태**: 월간 달력 그리드 대신 **주간/날짜별 리스트 뷰** 채택. 모바일에서 달력 셀이 너무 좁아 가독성 떨어짐.
