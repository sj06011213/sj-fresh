...
### 2026-04-26 (소진 기록 UX 통합 + 가계부 필터 + 폰트 톤다운 + 음주 카테고리 적용 누락 수정)

- **재료 카드 "소비" 버튼 → 소진 모달로 통합**: 기존엔 카드의 "소비" 버튼이 즉시 `consumed_at` 채워서 양·기록 없이 사라뜨림 ↔ 🍴 FAB는 양·기록을 남기는 정상 플로우. 두 동작이 너무 달라 혼란. → "소비" 버튼이 **그 재료가 미리 선택된 소진 모달**을 열도록 통합. FAB와 같은 모달 재사용. 잘못 눌렀을 때 모달 안에서 다른 재료로 바꾸거나 ✕로 닫기 가능.
  - 구현: `RecordUsageButton.tsx` → 모달은 `RecordUsageModal.tsx`로 분리(controlled), FAB는 `IngredientList`에 인라인. 즉시-소진용 server action `consumeIngredient`도 제거(미사용).
- **소진 모달 양 입력 단순화**: 단위 버튼(개/g/kg/ml/L/기타) 제거 → **숫자만 입력**. 단위는 선택한 재료의 `quantity`에서 자동 추출(`parseQuantity`). 입력칸 옆에 단위 라벨 회색 고정. 단위 없는 재료(예: "한 팩")만 자유 텍스트 폴백. 이유: 재료가 정해진 시점에 단위를 또 고르는 건 중복 — 단위 충돌 가능성도 사라져 자동 차감 신뢰성↑.

- **🐞 음주(alcohol) 등록 실패 버그**: 카테고리 5종을 위한 마이그레이션 `012_add_expense_alcohol_category.sql`은 만들어져 있었지만 **DB에 실제로 적용되지 않은 상태**였음. 음주로 기록 시 `violates check constraint` 에러. → `run-sql.mjs`로 적용. 교훈: 앱 코드와 DB 스키마 동기화 누락이 조용히 지나갈 수 있음 (현재 운영 규모에선 발생 시 수동 적용으로 충분).
- **가계부 카테고리 필터 추가**: 카테고리 비율 바를 **버튼화** → 탭 시 그 카테고리만 목록 표시. 별도 칩/드롭다운 UI 없이 기존 시각 요소를 재사용 (모바일 화면 절약). 선택 시 작은 안내 바 + ✕ 전체보기. 합계 금액은 전체 기준 유지(필터링은 목록에만 적용) — 카테고리별 합계는 비율 바에 이미 보임.
- **폰트 굵기 한 단계 톤다운**: 합계 금액 `font-bold` → `font-medium`, 정렬 옵션 선택 표시 `font-semibold` → `font-medium`, 필터 안내 바 카테고리 라벨 `<strong>` → `font-medium`. Jua(주아체)는 본래 굵은 인상이라 `bold/semibold`까지 가면 과해 보임 → 의도된 강조는 색·밑줄로, 굵기는 `medium` 선까지만.
